            ...
            if ($request->has('filter') && $request->get('filter') === 'count') {
                ...
                if ($request->has('filter') && in_array($request->get('filter'), ['lga', 'street'])) {
                    if ($request->get('filter') == 'street') {
                        ...
                        $individual_registrations = Individual::with('user')->where('street_id', $street_id)->paginate($per_page);
                    }
                    if ($request->get('filter') == 'lga') {
                        $local_government_area_id = $request->get('local_government_area_id');
                        $individual_registrations = Individual::with('user')->where('local_government_area_id', $local_government_area_id)->paginate($per_page);
                    }
                }
                return response()->json([
                    'status' => 'success',
